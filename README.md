...
- SOTA Gemini model: Gemini 2.5 Flash Preview and Gemini 2.5 Pro (Free tier API available)
  - Gemini 2.5 Pro (Default)
    - model name: `gemini-2.5-pro-preview-03-25`
    - Beware of the Free tier rate limit (should be more than enough for this project)
...
### 1. Search PubMed and retrieve abstracts
For prioritized gene-related articles. We only search for articles that have a specific gene in their title for the most relevant results and we only retrieve abstracts because of the access limit of using the API to scrape PubMed articles.

```shell
python pubmed_gene_search.py -l prioritized_gene_from_part1.txt
```

This will:
- Search top 10 PubMed abstracts for articles with the prioritized genes in the title
- Save the results to `pubmed_abstracts/<gene>_pubmed_abstracts.json`. For each abstract retrieved, it contains the following fields:
  - PMID
  - Title
  - Authors
  - Journal
...
Process all collected abstracts for the prioritized genes to construct a prompt embeed with the part II instruction for Gemini AI to conduct a comprehensive analysis as required
...
This will:
- Load all the abstracts from `pubmed_abstracts/`
...
- Save both the prompt and LLM response to files:
  - `prompts/integrated_prompt.txt`
  - `llm_responses/part2_text_mining_response.txt`


You can also use the `--check`/`-c` flag to check the generated prompt based on the abstracts first without further calling the API
...
### JSON Articles File
The PubMed search results are saved as a JSON file with detailed metadata for each article:
- PMID (PubMed ID)
- Title
- Authors
- Journal
...
### Analysis Prompt
The prompt file contains the structured input to the language model, including all article abstracts and analysis instructions.

### Analysis Response
The response file contains the Gemini-generated analysis with sections:
- Frequent biological terms
- Key findings about the gene
- Gene-disease associations
- Correlations between concepts
- Disease implications
- Comparison with database annotations
...
```
geneinsight/
├── pubmed_gene_search.py     # PubMed article retrieval script
├── llm_mining.py             # Gemini analysis script
├── .env                      # Environment variables (API key)
├── pubmed_abstracts/         # Directory for stored PubMed results
│   └── <gene>_pubmed_abstracts.json    # JSON files with article data
├── prompts/                  # Directory for generated prompts
│   └── integrated_prompt.txt
└── llm_responses/            # Directory for analysis results
    └── part2_text_mining_response.txt
```
